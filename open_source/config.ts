...
// @formatter:off
export function rappstack_dl_dt_tree_():[tag_dom_T, tag_dom_T] {
	const id = 'rappstack'
...
export function rmemo_md_():[tag_dom_T, tag_dom_T] {
	const id = 'rmemo'
...
// language=md
export function relementjs_md_():[tag_dom_T, tag_dom_T] {
	const id = 'relementjs'
...
// language=md
export function rebuildjs_md_():[tag_dom_T, tag_dom_T] {
	const id = 'rebuildjs'
...
// language=md
export function relysjs_md_():[tag_dom_T, tag_dom_T] {
	const id = 'relysjs'
...
// language=md
export function rebuildjs_tailwind_md_():[tag_dom_T, tag_dom_T] {
...
// language=md
export function hyop_md_():[tag_dom_T, tag_dom_T] {
	const id = 'hyop'
...
// language=md
export function ctx_core_md_(ctx_core_catch_all__footnote__sup_?:()=>tag_dom_T):[tag_dom_T, (tag_dom_T|((level:number)=>[tag_dom_T, tag_dom_T][]))[]] {
	const id = 'ctx-core'
...
// language=md
export function nanostores_md_(nanostores_breadth_first_queue__footnote__sup_?:()=>tag_dom_T):[tag_dom_T, tag_dom_T] {
	return [
...
// language=md
export function backbone_signal_md_():[tag_dom_T, tag_dom_T] {
	...
	return [
		`## ${backbone_signal__tb_a_({ id })}`, [
			description,
			nl,
			`I started using global state management with ${backbonejs__tb_a_()} while Lead Developer at ${honk__tb_a_()}. I created ${a_(
				{ href: '/posts/backbone-signal-practical-reactive-programming-javascript' },
				`backbone-signal`)} to add global state management to Backbone.js. I used this across different projects including ${rundavoo__tb_a_()}.`,
			nl,
			script_({ type: 'application/ld+json' }, raw_(JSON.stringify(<SoftwareSourceCode>{
				'@context': 'https://schema.org',
				'@type': 'SoftwareSourceCode',
...
// language=md
export function jasmine_flow_md_():[tag_dom_T, tag_dom_T] {
	...
	return [
		`## ${jasmine_flow__tb_a_({ id })}`, [
			description,
			nl,
			`At ${rundavoo__tb_a_()}, I created ${a_({ href: '/posts/jasmine-flow-fast-and-thourough-testing' },
				`jasmine-flow`)}. Jasmine-flow facilitates linear flow testing...saving effort in maintaining automated tests. This was hard-learned from maintaining various large apps while at ${honk__tb_a_()} & ${pivotal_labs__tb_a_()}.`,
			script_({ type: 'application/ld+json' }, raw_(JSON.stringify(<SoftwareSourceCode>{
				'@context': 'https://schema.org',
				'@type': 'SoftwareSourceCode',
				'@id': '#' + id,
...
// language=md
export function rr_md_():[tag_dom_T, tag_dom_T] {
	const id = 'rr'
	const name = rr_name
	const url = rr_href
	...
	return [
		`## ${rr__tb_a_({ id })}`, [
			description,
			nl,
			`I ${a_({ href: '/posts/introducing-rr' },
        `released RR`)} back in 2008. RR features a terse api to for a high signal to syntax noise ratio. Which allows a read & write friendly automated testing. I used global state management to manage complex data interactions. I stopped development when I moved to full-stack Javascript development. ${tb_a_(
        { href: 'https://github.com/kou', nofollow: true }, `Sutou Kouhei`)} began maintaining this project in 2015.`,
			script_({ type: 'application/ld+json' }, raw_(JSON.stringify(<SoftwareSourceCode>{
				'@context': 'https://schema.org',
				'@type': 'SoftwareSourceCode',
				'@id': '#' + id,
...
// language=md
export function rspec_md_(rspec_nested_describes__footnote__sup_?:()=>tag_dom_T):[tag_dom_T, tag_dom_T] {
	return [
		`## ${rspec__tb_a_({ id: 'rspec' })}`, [
			`I was an early core maintainer on the rspec team. Rspec was the first DSL (Domain Specific Language) implementation of ${tb_a_(
				{ href: 'https://dannorth.net/introducing-bdd/', nofollow: true },
				'BDD (Behavioral Driven Development)')}. Key contributions include the first known implementation of nested \`describe\` & \`context\` statements${rspec_nested_describes__footnote__sup_?.() ?? ''}. Nested describes were controversial at that time. Criticism included adding to the complexity & inhibit readability of the specs. Nesting \`describe\` & \`context\` enabled contextual nesting with separate usage of \`beforeEach\` & \`afterEach\`. I championed the idea & delivered the implementation. I integrated Rspec with ${tb_a_(
				{ href: 'https://ruby-doc.org/stdlib-3.0.0/libdoc/test-unit/rdoc/Test/Unit.html', nofollow: true },
				`Test::Unit`)}. Worked on the initial mock implementation. Worked on the ${ruby_on_rails__tb_a_()} integration.`]]
}
// language=md
export function desertrb_md_():[tag_dom_T, tag_dom_T] {
	return [
...
// language=md
export function jellyrb_md_():[tag_dom_T, tag_dom_T] {
	return [
...
// language=md
export function poloniumrb_md_():[tag_dom_T, tag_dom_T]  {
	return [
...
// language=md
export function unison_md_():[tag_dom_T, tag_dom_T]  {
	return [
...
// language=md
export function screw_unit_md_():[tag_dom_T, tag_dom_T] {
	return [
...
// language=md
export function generic_query_analyzer_md_():[tag_dom_T, tag_dom_T] {
	return [
...
// language=md
export function poof_md_():[tag_dom_T, tag_dom_T] {
	return [
		`## ${poof__tb_a_({ id: 'poof' })}`, [
